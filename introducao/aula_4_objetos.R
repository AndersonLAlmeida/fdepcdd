...
#no R: https://www.youtube.com/watch?v=ec0y7W2U74A&list=PLgnUrXr7_7coSfm067nFXPvShO18o6GQ_&index=5
...
# Fatores
xx <- factor(letters[1:10])   # fatores são categorias - vetor com níveis
factor(101:103)

# Conversão de fator para número
...
save.image(file = 'aula1.RData') # Salvando área de trabalho - útil para salvar informações de processamento de machine learning (ex: peso de um neurônio?)


# Vetores
(v <- c(2,0,1,2,4,2))   # Atribui o vetor (2,0,1,2,4,2) a 'v' e apresenta 'v'
2*v # multiplicação escalar, múltiplica cada elemento por 2
v^3 # eleva cada elemento à 3
v[3] # retorna a terceira posição do vetor
v[-3] # todo conteúdo de v, exceto a terceira posição
v[c(2,5)] # usa concatenação para obter as posições 2 e 5 de v
length(v) # tamanho de v

# numeric -> conjunto de números reais (flutuante)
# integer -> conjunto de números naturais (inteiros)
...
e1 <- vector()        # atribuindo um objeto 'vazio', aloca mais memória, evitar
...
identical(m1,t(m2))   # mais restritiva que 'all.equal', mesmo elemento e mesma classe
...
is.matrix(m3) #?matrix, byrow = FALSE (default)
...
m1 %*% m2             # o produto de matrizes é feito com %*% - multiplicação
crossprod(t(m1),m2)   # t(x) %*% y, mais rápido que %*%
m2 %*% m1
crossprod(m1,t(m2))


# Array - Matrizes multidimencionais
...
# Listas: podem incluir vetores,matrizes, arrays,fatores e outras listas
...
class(l1)
class(l1$matriz2)
diag(l1$matriz2) #diagonal
class(l1$char)
lapply(l1, class) # funciona como um looping aplicando a função class() em cada elemento da lista l1
l1$char[1]
names(l1) # nomes dos objetos da lista
...
fit <- lm(y ~ x) # lm: Fitting Linear Models (fit = ajuste)
names(fit) # nomes dos objetos da lista

fit$coefficients
fit$residuals
model.matrix(fit) # matriz do modelo
fit$effects
...
class(h)     # classe
dim(h)       # dimensão (linhas x colunas)
head(h)      # cabeçalho, 6 primeiros dados
tail(h)      # rodapé, 6 últimos dados
summary(h)   # função genérica de resumo
str(h)       # estrutura (structure)

# Data frame, diferença de matrix, possui colunas com quaisquer tipos de classes
